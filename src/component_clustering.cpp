...
void mapCartesianGrid(PointCloud<PointXYZI>::Ptr elevatedCloud, array<array<int, numGrid>, numGrid>& cartesianData)
{
  ...
  // Then all adjacent neighbor cells (i.e. x-1,y+1,x,y+1,x+1,y+1 x-1,y,x+1,y,x - 1, y -1, x, check the occupancy status
  // of y − 1, x + 1, y + 1) and mark it with the current cluster ID. Repeat this process for each x,y in the mxn grid
  // until all non-empty clusters have been assigned IDs.
...
void componentClustering(PointCloud<pcl::PointXYZI>::Ptr elevatedCloud, std::vector<pcl::PointIndices>& cluster_indices)
{
  ...
  getClusteredPoints(
      elevatedCloud, cartesianData, cluster_seed_, numCluster,
