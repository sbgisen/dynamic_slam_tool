...
void MovingObjectDetectionCloud::groundPlaneRemoval(float x, float y, float z)
{
  ...
  pcl::CropBox<pcl::PointXYZI> cropBoxFilter(
      true);  // class CropBox to filter out the point cloud data in the cube given by the user
    ...
    double delta)
{
  ...
  for (int j = 0; j < ufmp->size();
       j++)  // Check whether the volumes of the corresponding clusters of the two frames before and after match one by one
  {
